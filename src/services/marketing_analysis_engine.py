...
class MarketingAnalysisEngine:
    ...
    def analyze_client(self, client_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        insights = self._generate_insights(
            profile, sector_analysis, challenge_analysis, 
            opportunity_analysis, budget_analysis, competition_analysis
        )
        ...
        return {
            ...
            "desafios_solucoes": {
                "Baixa geração de leads": {
                    ...
                    "ferramentas": ["HubSpot", "RD Station", "Google Ads", "Facebook Ads"],
                ...
                "Conversão baixa": {
                    ...
                    "ferramentas": ["Hotjar", "Optimizely", "Google Optimize", "Unbounce"],
            ...
            "orcamento_estrategias": {
                ...
                "R$ 15.001 - R$ 50.000": {
                    "foco": ["Automação completa", "Múltiplos canais", "Personalização"],
        ...
        for desafio in profile.desafios:
            ...
            if solucao_data:
                insight = MarketingInsight(
                    ...
                    impacto_estimado=solucao_data.get('tempo_resultados', 'Médio prazo'),
                    complexidade=self._assess_complexity(solucao_data),
                    prazo_implementacao=solucao_data.get('tempo_resultados', '2-4 meses')
                )
        ...
        for canal in canais_recomendados:
            if canal not in profile.canais_atuais:
                opportunity = MarketingInsight(
                    ...
                    prioridade="Alta" if canal in ["Google Ads", "LinkedIn"] else "Média",
    ...
    def _analyze_budget(self, profile: ClientProfile) -> Dict[str, Any]:
        ...
        budget_data = self.knowledge_base["orcamento_estrategias"].get(profile.orcamento, {})
    ...
    def _analyze_competition(self, profile: ClientProfile) -> Dict[str, Any]:
        ...
        return {
            ...
            "principais_canais_concorrencia": self._get_competition_channels(profile.setor),
            "vantagens_competitivas": self._identify_competitive_advantages(profile),
            "estrategias_diferenciacao": self._suggest_differentiation_strategies(profile)
        }
    
    def _generate_insights(self, profile: ClientProfile, *analyses) -> List[MarketingInsight]:
        ...
        all_insights.sort(key=lambda x: priority_order.get(x.prioridade, 0), reverse=True)
        
        return all_insights
    
    def _calculate_scores(self, profile: ClientProfile, insights: List[MarketingInsight]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, profile: ClientProfile, insights: List[MarketingInsight]) -> List[str]:
    ...
    def _define_success_metrics(self, profile: ClientProfile) -> List[Dict[str, str]]:
        ...
        if profile.objetivo_principal == "Aumentar vendas":
            metrics.extend([
                {"metric": "Receita", "target": f"+{profile.meta_crescimento}%", "period": profile.prazo},
                {"metric": "Ticket médio", "target": "+15%", "period": profile.prazo},
                {"metric": "Taxa de conversão", "target": "+25%", "period": profile.prazo}
            ])
        
        elif profile.objetivo_principal == "Gerar mais leads":
            metrics.extend([
                {"metric": "Leads qualificados", "target": f"+{profile.meta_crescimento}%", "period": profile.prazo},
                {"metric": "Custo por lead", "target": "-20%", "period": profile.prazo},
                {"metric": "Taxa de conversão de leads", "target": "+30%", "period": profile.prazo}
            ])
    ...
    def _calculate_complexity_score(self, profile: ClientProfile, insights: List[MarketingInsight]) -> str:
    ...
    def _calculate_overall_priority(self, potential_score: float, complexity_score: str) -> str:
    ...
    def _find_budget_opportunities(self, profile: ClientProfile) -> List[MarketingInsight]:
    ...
    def _generate_combination_insights(self, profile: ClientProfile) -> List[MarketingInsight]:
        """Gerar insights da combinação de fatores"""
        insights = []
        
        # Insight baseado em setor + objetivo
        if profile.setor == "Representação Comercial" and profile.objetivo_principal == "Aumentar vendas":
            insight = MarketingInsight(
