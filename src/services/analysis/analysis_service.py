...
class AnalysisService:
    ...
    def set_data(self, df: pd.DataFrame) -> None:
        ...
        logger.info(
            f"Dados definidos para análise: {len(df)} linhas, "
    ...
    def descriptive_statistics(
        ...
        for col in numeric_cols:
            ...
            stats_dict[col] = {
                ...
                "coefficient_variation": (col_data.std() / col_data.mean()) * 100
                if col_data.mean() != 0
                else 0,
            }
    ...
    def categorical_analysis(
        ...
        for col in categorical_cols:
            ...
            analysis_dict[col] = {
                ...
                "most_frequent_percentage": (value_counts.iloc[0] / len(col_data)) * 100
                if len(value_counts) > 0
                else 0,
                ...
                "concentration_ratio": value_counts.iloc[0] / len(col_data)
                if len(value_counts) > 0
                else 0,
    ...
    def detect_outliers(
        ...
        for col in numeric_cols:
            ...
            outliers_dict[col] = {
                ...
                "outlier_values": self.data.loc[outliers, col].tolist()
                if len(outliers) > 0
                else [],
    ...
    def generate_insights(self) -> List[str]:
        ...
        if total_rows < 30:
            insights.append(
                "⚠️ Amostra pequena (< 30 registros). Resultados podem não ser representativos."
            )
        ...
        missing_percentage = (
            self.data.isnull().sum().sum() / (len(self.data) * len(self.data.columns))
        ) * 100
        if missing_percentage > 20:
            insights.append(
                f"⚠️ Alto percentual de dados faltantes ({missing_percentage:.1f}%). Considere melhorar a coleta."
            )
        elif missing_percentage < 5:
            insights.append(
                "✅ Baixo percentual de dados faltantes. Boa qualidade dos dados."
            )
        else:
            insights.append(
                f"📋 Percentual moderado de dados faltantes ({missing_percentage:.1f}%)."
            )
        ...
        try:
            ...
            if strong_corrs:
                insights.append(
                    f"🔗 Encontradas {len(strong_corrs)} correlações fortes entre variáveis numéricas."
                )
                if len(strong_corrs) > 3:
                    insights.append(
                        "💡 Muitas correlações fortes podem indicar multicolinearidade."
                    )
        except Exception as e:
        ...
        try:
            ...
            high_outlier_cols = [
                col for col, data in outliers.items() if data["outlier_percentage"] > 10
            ]
            if high_outlier_cols:
                insights.append(
                    f"🎯 Colunas com muitos outliers (>10%): {', '.join(high_outlier_cols)}"
                )
        except Exception as e:
    ...
    def market_research_insights(self) -> Dict[str, List[str]]:
        ...
        age_cols = [
            ...
            if "idade" in col.lower() or "age" in col.lower()
        ]
        if age_cols:
            age_col = age_cols[0]
        ...
        satisfaction_cols = [
            ...
            if any(
                word in col.lower()
                for word in ["satisfação", "satisfaction", "nota", "rating", "score"]
            )
        ]
        ...
        for col in satisfaction_cols:
            if pd.api.types.is_numeric_dtype(self.data[col]):
                ...
                if satisfaction_percentage < 60:
                    insights["satisfaction_insights"].append(
                        f"⚠️ Baixa satisfação em {col}"
                    )
                elif satisfaction_percentage > 80:
                    insights["satisfaction_insights"].append(
                        f"✅ Alta satisfação em {col}"
                    )
        ...
        insights["data_quality"].append(
            f"Completude dos dados: {100 - missing_percentage:.1f}%"
        )

        duplicate_percentage = (self.data.duplicated().sum() / len(self.data)) * 100
        insights["data_quality"].append(
            f"Registros únicos: {100 - duplicate_percentage:.1f}%"
        )

        # Recomendações
        if len(self.data) < 100:
            insights["recommendations"].append(
                "Aumentar amostra para pelo menos 100 respondentes"
            )
        ...
        insights["recommendations"].append(
            "Implementar segmentação baseada em clustering"
        )
        insights["recommendations"].append(
            "Desenvolver análise de sentimentos para feedback qualitativo"
        )
        insights["recommendations"].append(
            "Criar dashboards interativos para monitoramento contínuo"
        )

        return insights

    def statistical_tests(
        self, column1: str, column2: str = None, test_type: str = "auto"
    ) -> Dict[str, Any]:
        """
            ...
            if column2 is None:
                ...
                if test_type == "auto":
                    if pd.api.types.is_numeric_dtype(
                        self.data[column1]
                    ) and pd.api.types.is_numeric_dtype(self.data[column2]):
                        # T-test para variáveis numéricas
                        stat, p_value = stats.ttest_ind(
                            self.data[column1].dropna(), self.data[column2].dropna()
                        )
                        result = {
                        ...
                        result = {
                            ...
                            "statistic": stat,
                            "p_value": p_value,
                            "interpretation": "Associação significativa"
                            if p_value < 0.05
